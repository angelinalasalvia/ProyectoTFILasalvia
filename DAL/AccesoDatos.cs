...
namespace DAL
{
    ...
    public class AccesoDatos : IAccesoDatos
    {
        ...
        public async Task<IEnumerable<T>> Leer<T>(string consulta, object? parametros = null, bool esStoreProcedure = false, CancellationToken ct = default) where T : new()
        {
            ...
            // Mapeamos las columnas del reader a propiedades de T por nombre (case-insensitive).
            ...
            while (await lector.ReadAsync(ct))
            {
                ...
                {
                    var nombreColumna = columnas.FirstOrDefault(c => string.Equals(c, propiedad.Name, StringComparison.OrdinalIgnoreCase));
                    if (nombreColumna is null) continue;

                    var valor = lector[nombreColumna];
                    if (valor is DBNull) continue;
        ...
        public Task<int> Escribir(string consulta, object? parametros = null, bool esStoreProcedure = false, CancellationToken ct = default)
            => EjecutarNonQuery(consulta, parametros, esStoreProcedure, ct);

        public Task<int> Modificar(string consulta, object? parametros = null, bool esStoreProcedure = false, CancellationToken ct = default)
            => EjecutarNonQuery(consulta, parametros, esStoreProcedure, ct);

        public Task<int> Eliminar(string consulta, object? parametros = null, bool esStoreProcedure = false, CancellationToken ct = default)
            => EjecutarNonQuery(consulta, parametros, esStoreProcedure, ct);

        private async Task<int> EjecutarNonQuery(string consulta, object? parametros, bool esStoreProcedure, CancellationToken ct)
        {
            await using var conexion = new SqlConnection(_cadenaConexion);
            await conexion.OpenAsync(ct);
            ...
            try
            {
                await using var comando = new SqlCommand(consulta, conexion, transaccion)
                {
                    CommandType = esStoreProcedure ? CommandType.StoredProcedure : CommandType.Text
                };
        ...
        // Convierte un objeto anónimo (o cualquier POCO) en parámetros @NombrePropiedad.
        // Ej: AgregarParametros(cmd, new { idCliente = 5 }) -> agrega @idCliente = 5
        private static void AgregarParametros(SqlCommand comando, object? parametros)
        {
            ...
            if (parametros is IDictionary<string, object?> diccionario)
            {
                foreach (var (clave, valor) in diccionario)
                    comando.Parameters.AddWithValue($"@{clave}", valor ?? DBNull.Value);
                return;
            }

            foreach (var propiedad in parametros.GetType().GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                var valor = propiedad.GetValue(parametros) ?? DBNull.Value;
